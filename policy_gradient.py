...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      log_std = tf.get_variable('logstd', shape = [self.action_dim,], dtype=tf.float32, trainable=True)
      std = tf.exp(log_std)
      self.sampled_action = tf.random_normal(tf.shape(action_means), mean = action_means, stddev = std)
      dist = tf.contrib.distributions.MultivariateNormalDiag(action_means, std)
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      T = len(rewards)
      for i in range(T):
          gammas = np.logspace(0, T - i, num = T - i, base = self.config.gamma, endpoint = False)
